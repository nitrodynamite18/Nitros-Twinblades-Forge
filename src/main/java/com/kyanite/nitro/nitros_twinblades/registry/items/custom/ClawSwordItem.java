...
public class ClawSwordItem extends SwordItem {
    public ClawSwordItem() {
        super(Tiers.NETHERITE, 3, -2.4f, new Item.Properties().fireResistant().rarity(Rarity.create("claw", ChatFormatting.GOLD)).stacksTo(1).tab(CreativeModeTab.TAB_COMBAT));
    }
    ...
    public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, List<Component> components, @NotNull TooltipFlag flag) {
        components.add(Component.translatable("tooltip.nitros_twinblades.claw_sword").withStyle(ChatFormatting.GOLD));
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, @NotNull LivingEntity attacker) {
        ...
        EvokerFangs fangs = EntityType.EVOKER_FANGS.create(target.level);
        assert null != fangs;
        fangs.setOwner(attacker);
        fangs.moveTo(target.getX(), Math.floor(target.getY()), target.getZ());
